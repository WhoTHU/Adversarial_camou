...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput):
        ...
        if self.name == 'yolov2':
            ...
            output = output.view(batch, 5 + self.num_cls , 5 * h * w)  # [batch, 85, 1805] (1805 anchors in total)
            ...
            confs_if_object = self.func(output_objectness, confs_for_class) # [batch, 1805]
            # func = lambda obj, cls: obj, the objective is only to minimize the objectness confidence.
...
class PatchTransformer(nn.Module):
    ...
    def __init__(self):
        ...
        for i in range(3):
            kernel[i, i] = x
        self.register_buffer('kernel', kernel) # gaussian
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True, pooling='gauss'):
        ...
        adv_batch = adv_patch.expand(lab_batch.size(0), lab_batch.size(1), -1, -1, -1) # lab_batch: (N, num_objects, 5)
